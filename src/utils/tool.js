...
// 文件上传
tool.upload = {
  file: () => `${tool.baseUrl}/v1/upload/matchOups?accessToken=${tool.sessionGet('accessToken')}`,
  movie: () => `${tool.baseUrl}/v1/upload/matchVideo?accessToken=${tool.sessionGet('accessToken')}`,
  file2: () => `${tool.baseUrl}/v1/upload/matchAttachment?accessToken=${tool.sessionGet('accessToken')}`
}
/* 检查状态码 */
...
/* 正确提示 */
tool.messageSuccess = (message, sec = 5) => {
...
/* 错误提示 */
tool.messageError = (message, sec = 5) => {
...
tool.sessionSet = (key, data) => {
  typeof data !== 'object' && sessionStorage.setItem(key, data)
  typeof data === 'object' && sessionStorage.setItem(key, JSON.stringify(data))
}
/* 移除 */
tool.localRem = (key) => {
